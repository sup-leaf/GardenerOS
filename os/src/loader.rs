...
pub fn load_apps() {
    ...
    for i in 0..num_app {
        ...
        println!("[loader] Loaded app {} at {:#x}", i, base_i);
    }
}

pub fn init_app_cx(app_id: usize) -> &'static TaskContext {
    println!("[loader] init_app_cx for app {}", app_id);
    let base_i = get_base_i(app_id);
    let sp = USER_STACK[app_id].get_sp();
    println!("[loader] base: {:#x}, sp: {:#x}", base_i, sp);
    let trap_cx = TrapContext::app_init_context(base_i, sp);
    let task_cx = TaskContext::goto_restore();
